...
{stack}
        """)
        count +=1
        if node.has_left_child() and not node.get_left_child().get_value() in visit_order:
            node = node.get_left_child()
            stack.push(node)
        ...
        elif node.has_right_child()and not node.get_right_child().get_value() in visit_order:
            node = node.get_right_child()
            stack.push(node)
            node = stack.top()
            visit_order.append(node.get_value())
